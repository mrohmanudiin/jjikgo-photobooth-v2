...
export function PromoManagement() {
    ...
    const [formData, setFormData] = useState({ label: '', discount: '', type: 'percentage', active: true });
    ...
    const fetchPromos = useCallback(async () => {
        setLoading(true);
        try {
            const res = await api.get('/studio/promos');
            setPromos(res.data || []);
        } catch (err) {
    ...
    const handleSave = async () => {
        ...
        try {
            if (editingId) {
                await api.put(`/studio/promos/${editingId}`, formData);
            } else {
                await api.post('/studio/promos', formData);
            }
            setShowForm(false);
            setEditingId(null);
            setFormData({ label: '', discount: '', type: 'percentage', active: true });
            fetchPromos();
        } catch (err) {
    ...
    const handleEdit = (promo) => {
        ...
        setFormData({ label: promo.label, discount: promo.discount, type: promo.type, active: promo.active !== false });
        setShowForm(true);
    };
        ...
        <div className="space-y-6 animate-in fade-in duration-500">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-muted-foreground mt-1">Manage discounts and promotions available in the cashier app.</p>
                </div>
                <Button onClick={() => { setEditingId(null); setFormData({ label: '', discount: '', type: 'percentage', active: true }); setShowForm(true); }}>
